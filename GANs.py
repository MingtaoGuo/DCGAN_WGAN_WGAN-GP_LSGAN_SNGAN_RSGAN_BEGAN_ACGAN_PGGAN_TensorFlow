...
img_W = 64
img_C = 3
GAN_type = "DCGAN"  # DCGAN, WGAN, WGAN-GP, SNGAN, LSGAN
batchsize = 128
epsilon = 1e-8
    ...
    def __call__(self):
        ...
        for epoch in range(epoch_nums):
            for i in range(facedata.__len__()//batchsize-1):
                ...
                # for j in np.arange(start=i*batchsize, stop=i*batchsize+batchsize):
                #     batch[j-i*batchsize, :, :, :] = np.reshape(misc.imresize(np.array(Image.open(path + filenames[j])), [img_H, img_W]),[img_H, img_W, img_C])
                batch = facedata[i*batchsize:i*batchsize+batchsize, :, :, :] / 128.0 - 1.0
